...
# This script analyzes and visualizes the correlation between normalized death rate differences 
# (unvaccinated - vaccinated) and the vaccination doses administered, grouped by age bins over time.

# Key steps:
# - Load vaccination and death data per individual
# - Preprocess to extract age and vaccine/death event timings
# - Classify deaths as vaccinated or unvaccinated at time of death
# - Estimate vaccinated/unvaccinated population size per day and age bin
# - Compute normalized deaths per 100k for vx and uvx groups
# - Calculate the difference between normalized deaths
# - Compute smoothed versions and rolling Pearson correlation with doses
# - Plot time series of correlation, p-values, death differences, and doses for each age bin

# Output:
# - A Plotly HTML file visualizing the above metrics.

# --- Config ---
REFERENCE_DATE = pd.Timestamp("2020-01-01")  # Start of timeline for relative day calculations
INPUT_CSV = r"C:\CzechFOI-DRATE\TERRA\Vesely_106_202403141131.csv"  # Input file path
OUTPUT_HTML = r"C:\CzechFOI-DRATE\Plot Results\UC) diff norm death dose agebin\norm rolling_corr_doses_vs_diff.html"  # Output HTML for Plotly
start_delay = 0  # Delay to shift dose effect relative to death day
AGE_BIN_WIDTH = 1  # Bin width for age groups (e.g., 1 year)
ROLLING_WINDOW = 60  # Smoothing window (in days)
CORR_WINDOW = 30  # Window size for rolling correlation
...
df["age"] = 2023 - df["Rok_narozeni"]  # Calculate age
df = df[df["age"].between(0, 113)].copy()  # Filter valid ages
...
df_deaths = df.dropna(subset=["t_death"]).copy()  # Filter individuals with known death date

# Check if any dose was before death (adjusted by delay), flag as vaccinated
...
# Aggregate deaths per age_bin/day/vx_status
...
df["first_vax_day"] = df[dose_date_cols].min(axis=1)  # First dose date per individual
df["first_vax_day"] = df["first_vax_day"].fillna(1e9).astype(int)  # Use large number for unvaccinated
df["death_day"] = df["t_death"].fillna(1e9).astype(int)  # Use large number for alive people

# Create time axis
days = np.arange(0, int(df[["first_vax_day", "death_day"]].replace(1e9, np.nan).max().max()) + 1)
pop_data = []

# Count vaccinated/unvaccinated alive individuals per day/age_bin
...
agg = agg.fillna({col: 0 for col in agg.select_dtypes(include=["number"]).columns})  # Fill missing with zeros

# Normalize death rates (per 100k)
...
agg["deaths_diff_norm"] = agg["deaths_uvx_norm"] - agg["deaths_vx_norm"]  # Difference of normalized rates

# --- Doses ---
# Unpivot dose dates into rows
df_doses = df.melt(
    id_vars=["age_bin"],
...
# Convert dose days to integer day + optional delay
...
agg = agg.fillna({col: 0 for col in agg.select_dtypes(include=["number"]).columns})  # Fill missing with 0
agg["age_bin"] = agg["age_bin"].astype(str)  # Ensure string type for plotting

# --- Plotting ---
fig = go.Figure()

# Loop through each age bin to compute and plot metrics
for age_bin, group in agg.groupby("age_bin"):
    group = group.sort_values("day").reset_index(drop=True)
    
    # Smooth death diff and doses with rolling mean
    ...
    for i in range(len(group) - CORR_WINDOW + 1):
        ...
        corr_vals.append(r)
        pvals.append(p)
        corr_days.append(days[i + CORR_WINDOW // 2])  # Center day for window
    ...
    # Plot significant p-value marker (1 if p < 0.05)
    fig.add_trace(go.Scatter(
        x=corr_days,
    ...
    # Plot smoothed death difference (secondary y-axis)
    fig.add_trace(go.Scatter(
        x=group["day"],
    ...
    # Plot smoothed doses given (tertiary y-axis)
    fig.add_trace(go.Scatter(
        x=group["day"],
